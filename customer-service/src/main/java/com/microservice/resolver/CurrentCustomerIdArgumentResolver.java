...
public class CurrentCustomerIdArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CurrentCustomerIdArgumentResolver.class);
    ...
    public Object resolveArgument(MethodParameter parameter,
        ...
        logger.debug("Resolving customer ID from Authorization header using @CurrentCustomerId annotation");

        if (authHeader == null) {
            throw new RuntimeException("Authorization header is required");
        }

        return jwtUtil.getCustomerIdFromHeader(authHeader);
    }
}